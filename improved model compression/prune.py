...
from models.pruned_common import SPPCSPCPruned
import numpy as np
import torch
...
def run_prune(data,
    ...
    for bnname, bnlayer in model.named_modules():
        ...
        if isinstance(bnlayer, nn.BatchNorm2d):
            ...
            if "rbr_dense" in bnname:
                rbr_dense_mask = mask
                rbr_dense_bnname = bnname
                rbr_dense_bn_moudle = bn_module
                continue

            if "rbr_1x1" in bnname:
                mask = rbr_dense_mask + mask
                for i in range(mask.shape[0]):
                    if mask[i] > 1.:
                        mask[i] = 1.

                maskbndict[rbr_dense_bnname] = mask
                maskbndict[bnname] = mask

                remain_num += 2 * int(mask.sum())

                rbr_dense_bn_moudle.weight.data.mul_(mask)
                rbr_dense_bn_moudle.bias.data.mul_(mask)
                bn_module.weight.data.mul_(mask)
                bn_module.bias.data.mul_(mask)

                print(f"|\t{rbr_dense_bnname:<25}{'|':<10}{rbr_dense_bn_moudle.weight.data.size()[0]:<20}{'|':<10}{int(mask.sum()):<20}|")
                assert int(
    ...
    for ((layername, layer), (pruned_layername, pruned_layer)) in zip(model.named_modules(),
        ...
        if isinstance(layer, nn.Conv2d) and not layername.startswith("model.105"):
            if "rbr" in layername:
                convname = layername[:-1] + "1"
            else:
                convname = layername[:-4] + "bn"
            if convname in from_to_map.keys():
                former = from_to_map[convname]
                if isinstance(former, str):
                    out_idx = np.squeeze(np.argwhere(np.asarray(maskbndict[convname].cpu().numpy())))
        ...
        if isinstance(layer, nn.Conv2d) and layername.startswith("model.105"):
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/panel.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'best_Q.pt',
                        help='model.pt path(s)')
    parser.add_argument('--cfg', type=str, default='cfg/deploy/yolov7.yaml', help='model.yaml path')
    parser.add_argument('--percent', type=float, default=percent, help='prune percentage')  # 一般写0.5就好，50%剪枝
